...
function placeSportsBet() {
    ...
    function saveBetToActive() {
    const currency = localStorage.getItem('selectedCurrency') || 'USDT';
    const balanceStr = localStorage.getItem('selectedBalance') || '$0.00';
    const currencySymbol = balanceStr.match(/[^0-9.,]/g)?.[0] || '$';
    ...
    betSlip.forEach(bet => {
        activeBets.unshift({
            ...
            placedAt: new Date().toLocaleString('en-GB', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' }),
            status: 'active'
        });
    });
...
let sportsHistory = [];
let casinoHistory = [];
let historyMode = 'sports'; // 'sports' | 'casino'
let sportsSubTab = 'active'; // 'active' | 'history'

// Load from localStorage
if (localStorage.getItem('sportsActiveBets')) {
    try { activeBets = JSON.parse(localStorage.getItem('sportsActiveBets')); } catch(e) {}
}
if (localStorage.getItem('sportsHistory')) {
    try { sportsHistory = JSON.parse(localStorage.getItem('sportsHistory')); } catch(e) {}
}

// ===== ১. OPEN BET HISTORY MODAL =====
...
function switchHistoryMode(mode) {
    ...
    // Update tab styles
    document.querySelectorAll('.history-mode-tab').forEach(t => t.classList.remove('active'));
    const tabs = document.querySelectorAll('.history-mode-tab');
    if (mode === 'sports' && tabs[0]) tabs[0].classList.add('active');
    if (mode === 'casino' && tabs[1]) tabs[1].classList.add('active');
...
function switchSportsSubTab(tab) {
    ...
    document.querySelectorAll('.sports-sub-tab').forEach(t => t.classList.remove('active'));
    const tabs = document.querySelectorAll('.sports-sub-tab');
    if (tab === 'active' && tabs[0]) tabs[0].classList.add('active');
    if (tab === 'history' && tabs[1]) tabs[1].classList.add('active');
    
    renderHistoryTabs();
}

// ===== ৩. RENDER CONTENT BASED ON TAB =====
function renderHistoryTabs() {
    const container = document.getElementById('historyContentArea');
    if (!container) return;
    
    if (historyMode === 'sports') {
        // Sports: Show Active Bets + Bet History sub-tabs
        let html = `
            <div class="sports-sub-tabs">
                <button class="sports-sub-tab ${sportsSubTab==='active'?'active':''}" onclick="switchSportsSubTab('active')">Active Bets</button>
                <button class="sports-sub-tab ${sportsSubTab==='history'?'active':''}" onclick="switchSportsSubTab('history')">Bet History</button>
            </div>
            <div class="history-list" id="sportsHistoryList"></div>
        `;
        container.innerHTML = html;
        
        if (sportsSubTab === 'active') {
            renderActiveBets();
        } else {
            renderBetHistory();
        }
    } else {
        // Casino: My Bets + Top Winner
        let html = `
            <div class="casino-section">
                <div class="casino-option" onclick="showCasinoMyBets()">My Bets</div>
                <div class="casino-option" onclick="showCasinoTopWinners()">Top Winners</div>
            </div>
            <div class="history-list" id="casinoHistoryList"></div>
        `;
        container.innerHTML = html;
        showCasinoMyBets();
    }
...
// ===== ৪. RENDER ACTIVE BETS (with Cash Out) =====
function renderActiveBets() {
    ...
    activeBets.forEach(bet => {
        const cashAmount = (bet.stake * bet.odds * 0.85).toFixed(2);
        const potential = (bet.stake * bet.odds).toFixed(2);
        
        html += `
            <div class="history-bet-card active-bet">
                <div class="bet-status-badge live">LIVE</div>
                ...
                <div style="margin-top:6px;display:flex;justify-content:space-between;align-items:center;">
                    <span>Stake: ${bet.currencySymbol}${bet.stake.toFixed(2)}</span>
                    <span style="color:#4ade80;">Potential: ${bet.currencySymbol}${potential}</span>
                </div>
                <div style="margin-top:8px;display:flex;gap:8px;">
                    <button class="btn-cashout" onclick="cashOutBet('${bet.id}')">
                        Cash Out ${bet.currencySymbol}${cashAmount}
                    </button>
                </div>
            </div>
        `;
...
function renderBetHistory() {
    ...
    sportsHistory.forEach(bet => {
        ...
        const statusText = bet.status === 'won' ? 'Won' : 
                          bet.status === 'lost' ? 'Lost' : 'Cashed Out';
        const statusColor = bet.status === 'won' ? '#4ade80' : 
                           bet.status === 'lost' ? '#ff4444' : '#ff6b00';
        
        html += `
            <div class="history-bet-card ${statusClass}">
                <div class="bet-status-badge" style="background:${statusColor};">${statusText}</div>
                <div style="font-weight:bold;">${bet.eventName}</div>
                <div style="color:#888;font-size:12px;">${bet.market} | @${bet.odds.toFixed(2)}</div>
                <div style="margin-top:4px;display:flex;justify-content:space-between;">
                    <span>Stake: ${bet.currencySymbol}${bet.stake.toFixed(2)}</span>
                    <span>Return: ${bet.currencySymbol}${bet.returnAmount.toFixed(2)}</span>
                </div>
                <div style="color:#666;font-size:11px;margin-top:4px;">${bet.settledAt || ''}</div>
            </div>
        `;
    });
...
function cashOutBet(betId) {
    ...
    const cashAmount = bet.stake * bet.odds * 0.85;
    
    if (!confirm(`Cash out ${bet.currencySymbol}${cashAmount.toFixed(2)} now?`)) return;
    ...
    sportsHistory.unshift({
        ...
        settledAt: new Date().toLocaleString('en-GB', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' })
    });
    ...
    // Refund balance (cash out amount)
    let balanceStr = localStorage.getItem('selectedBalance') || '$0.00';
    let currencySymbol = balanceStr.match(/[^0-9.,]/g)?.[0] || '$';
    let currentBalance = parseFloat(balanceStr.replace(/[^0-9.]/g, '')) || 0;
    let newBalance = currentBalance + cashAmount;
    let newBalanceStr = currencySymbol + newBalance.toFixed(2);
    localStorage.setItem('selectedBalance', newBalanceStr);
    
    // Update UI
    updateSlipBalance();
    updateMainWalletUI(bet.currency, newBalanceStr);
    renderActiveBets();
    
    showToast(`Cashed out ${bet.currencySymbol}${cashAmount.toFixed(2)}!`, 'success');
}

// ===== ৭. SETTLE BET (Win/Lose) - for admin or auto-settle =====
function settleBet(betId, result) {
    ...
    // If won, add return to balance
    if (result === 'win') {
        let balanceStr = localStorage.getItem('selectedBalance') || '$0.00';
        let currencySymbol = balanceStr.match(/[^0-9.,]/g)?.[0] || '$';
        let currentBalance = parseFloat(balanceStr.replace(/[^0-9.]/g, '')) || 0;
        let newBalance = currentBalance + returnAmount;
        let newBalanceStr = currencySymbol + newBalance.toFixed(2);
        localStorage.setItem('selectedBalance', newBalanceStr);
        updateSlipBalance();
        updateMainWalletUI(bet.currency, newBalanceStr);
    ...
    showToast(`Bet ${status}!`, status === 'won' ? 'success' : 'info');
}

// ===== ৮. MODIFY placeSportsBet() TO SAVE ACTIVE BETS =====
// তোমার আগের placeSportsBet() এর শেষে, betSlip = [] করার আগে এটা যোগ করো:
